...
imageList = newArray(100000);
redList = newArray(100000);
greenList = newArray(100000);
colocList = newArray(100000);
offsetUsed = newArray(100000);
lowerRedT = newArray(100000);
upperRedT = newArray(100000);
lowerGreenT = newArray(100000);
upperGreenT = newArray(100000);
imageScale = newArray(100000);
imageUnit = newArray(100000);
iterator = 0;
...
if (analysisType == "3 Channel Colocalization (RGB)"){
	//arrays to store values for the summary ouput
	imageList2 = newArray(100000);
	blueList = newArray(100000);
	colocListRtoB = newArray(100000);
	colocListGtoB = newArray(10000);
	colocListTriple = newArray(100000);
	lowerBlueT = newArray(100000);
	upperBlueT = newArray(100000);
	offsetUsed2 = newArray(100000);
	iterator2 = 0;
...
function thirdPuncta(dir1, dir2, blueMinPixel, roiType, imageList2, blueList, colocList2, lowerBlueT, upperBlueT, iterator2, roiX, roiY, roiR, currentOffset, offsetUsed2, threshType, ilpBlueDir) {
	...
	open(dir2 + splitName[0] + "_coloc_binary" + currentOffset + ".tiff");
	selectWindow(splitName[0] + "_coloc_binary" + currentOffset);
	rename("colocRG");

	//Run image calculator for RG colocs and blue 

	imageCalculator("AND create", "blue_thresholded", "colocRG");
	rename(title + "_colocsRGB");
	...
	//saves the output results table of Coloc_Calc, all the coloc puncta
	//coordinates, to a .csv file "colocResults"
	saveAs("Results", dir2 + splitName[0] + "_RGBcolocResults_" + toString(currentOffset) + ".csv");

	selectWindow(title + "_colocs");
	saveAs("tiff", dir2 + splitName[0] + "_RGBcoloc_binary");
	//stores the values from the Coloc_Calc plugin output Results table
	//as arrays
	selectWindow("Results");
	colocX2 = Table.getColumn("X");
	colocY2 = Table.getColumn("Y");
	colocArea2 = Table.getColumn("Area");
	colocCountRGB = colocX2.length;

	//saves the output results table of Coloc_Calc, all the coloc puncta
	//coordinates, to a .csv file "colocResults"
	saveAs("Results", dir2 + splitName[0] + "_colocResultsRGB_" + toString(currentOffset) + ".csv");
	...
	run("Select None");
	print("colocCount is " + colocCountRGB);
	...
	//Saves the feedback image showing colocalized puncta counted
	saveAs("tiff", dir2 + splitName[0] + "_colocsRGB");
	close("Results");
	close("oldResults");
	run("Close All");
	close("ROI Manager");
	...
	colocListRGB[iterator2] = colocCountRGB;
