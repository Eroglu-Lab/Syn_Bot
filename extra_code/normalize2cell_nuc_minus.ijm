...
colocArray = Table.getColumn("colocsPunctaCount");
...
for (i = 0; i < list1.length; i++) {
	...
	if (endsWith(currentFile, "results.csv")){
		continue;
	}
	...
	if (endsWith(currentFile, "rois.zip")){
		continue;
	}
	
	if (endsWith(currentFile, "greenThresholded.tiff")){
		continue;
	}
	
	if (endsWith(currentFile, "redThresholded.tiff")){
		continue;
	}
	if (endsWith(currentFile, "mask1.tif")){
		continue;
	}
...
for (i = 0; i < numImgs; i++) {
	...
	normRedArray[i] = Table.get("redPunctaCount", i) / Table.get("Territory Area", i);
	normGreenArray[i] = Table.get("greenPunctaCount", i) / Table.get("Territory Area", i);
	normColocArray[i] = Table.get("colocsPunctaCount", i) / Table.get("Territory Area", i);
