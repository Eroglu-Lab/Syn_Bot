...
#initialize genotype
colocDataIn$genotype <- "blank"

#label data with experimental condition
colocDataIn[which(grepl("WT", colocDataIn$Image)),]$genotype <- "WT"
colocDataIn[which(grepl("GS", colocDataIn$Image)),]$genotype <- "GS"

#make genotype a factor so that it always plots in WT then GS order
colocDataIn$genotype <- factor(colocDataIn$genotype, levels = c("WT", "GS"))

colocDataIn$cell <- -99
cell_num <- 1

#start with first cell
previous_cell <- str_split(colocDataIn$Image[1], "-")[[1]][2]

#average the z_stacks for each cell
for (i in 1:length(colocDataIn$Image)) {
  
  current_cell <- str_split(colocDataIn$Image[i], "-")[[1]][2]
  if(current_cell == previous_cell){
    colocDataIn$cell[i] <- cell_num
  }
  if(current_cell != previous_cell){
    cell_num <- cell_num + 1
    colocDataIn$cell[i] <- cell_num
...
write.csv(colocDataIn, "F:/GitHub/20240215_shiyi_analysis/20240410_lc3b_p62_colocalized_puncta_area_v2.csv")


redDataIn <- read.csv("F:/GitHub/20240215_shiyi_analysis/20240410_lc3b_puncta_area.csv")

#initialize genotype
redDataIn$genotype <- "blank"

#label data with experimental condition
redDataIn[which(grepl("WT", redDataIn$Image)),]$genotype <- "WT"
redDataIn[which(grepl("GS", redDataIn$Image)),]$genotype <- "GS"

#make genotype a factor so that it always plots in WT then GS order
redDataIn$genotype <- factor(redDataIn$genotype, levels = c("WT", "GS"))

redDataIn$cell <- -99
cell_num <- 1

#start with first cell
previous_cell <- str_split(redDataIn$Image[1], "-")[[1]][2]

#average the z_stacks for each cell
for (i in 1:length(redDataIn$Image)) {
  
  current_cell <- str_split(redDataIn$Image[i], "-")[[1]][2]
  if(current_cell == previous_cell){
    redDataIn$cell[i] <- cell_num
  }
  if(current_cell != previous_cell){
    cell_num <- cell_num + 1
    redDataIn$cell[i] <- cell_num
...
write.csv(redDataIn, "F:/GitHub/20240215_shiyi_analysis/20240410_lc3b_puncta_area_v2.csv")

greenDataIn <- read.csv("F:/GitHub/20240215_shiyi_analysis/20240410_p62_puncta_area.csv")

#initialize genotype
greenDataIn$genotype <- "blank"

#label data with experimental condition
greenDataIn[which(grepl("WT", greenDataIn$Image)),]$genotype <- "WT"
greenDataIn[which(grepl("GS", greenDataIn$Image)),]$genotype <- "GS"

#make genotype a factor so that it always plots in WT then GS order
greenDataIn$genotype <- factor(greenDataIn$genotype, levels = c("WT", "GS"))

greenDataIn$cell <- -99
cell_num <- 1

#start with first cell
previous_cell <- str_split(greenDataIn$Image[1], "-")[[1]][2]

#average the z_stacks for each cell
for (i in 1:length(greenDataIn$Image)) {
  
  current_cell <- str_split(greenDataIn$Image[i], "-")[[1]][2]
  if(current_cell == previous_cell){
    greenDataIn$cell[i] <- cell_num
  }
  if(current_cell != previous_cell){
    cell_num <- cell_num + 1
    greenDataIn$cell[i] <- cell_num
...
write.csv(greenDataIn, "F:/GitHub/20240215_shiyi_analysis/20240410_p62_puncta_area_v2.csv")
```
