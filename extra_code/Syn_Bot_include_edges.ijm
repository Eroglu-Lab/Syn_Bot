...
function analyzePuncta(dir1, dir2, currentOffset, redMinPixel, greenMinPixel, blueMinPixel, roiType, ilpRedDir, ilpGreenDir, ilpBlueDir){
	...
	run("Analyze Particles...", "size=" + redMinPixel + "-" + redMaxString + " pixel show=Overlay display clear summarize add");
	close("Summary");
	...
	//runs Analyze Particles plugin using minPixel
	run("Analyze Particles...", "size=" + greenMinPixel + "-"+ greenMaxString +" pixel show=Overlay display clear summarize add");
	close("Summary");
	...
	if (channelType == "3-Channel Colocalization (RGB)"){
		...
		*/
		...
		roiManager("reset");
		//runs Analyze Particles plugin using minPixel
		run("Analyze Particles...", "size=" + blueMinPixel + "-" + blueMaxString + " pixel show=Overlay display clear summarize add");
		close("Summary");
	...
	if (analysisType == "Pixel-overlap" && channelType == "2-Channel Colocalization (RG)"){
		...
		run("Analyze Particles...", "size=" + colocMinPixel + "-Infinity pixel show=Overlay display clear summarize add");
		close("Summary");
...
function autoCellBody(dir1){
	...
	run("Analyze Particles...", "  show=Ellipses display clear summarize add");
