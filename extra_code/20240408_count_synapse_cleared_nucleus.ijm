...
for (i = 0; i < list1.length; i++) {
	...
	open(redThreshPath);
	roiManager("reset");
	//analyze whole image with nuclear roi cleared
	open(roiPath);
	//clear nuclear signal
	roiManager("Select", 0);
	run("Clear");
	run("Select All");
	
	//counts synapses in each ROI and adds them to the Summary results table
    run("Analyze Particles...", "size="+redMinPixel+"-Infinity pixel show=Overlay display exclude clear summarize");
    
    //save redResults
	selectWindow("Results");
	saveAs("Results", dirOut + splitName[0] + "_redResults.csv");
    ...
    //counts synapses in each ROI and adds them to the Summary results table
    run("Analyze Particles...", "size="+greenMinPixel+"-Infinity pixel show=Overlay display exclude clear summarize");
	
	//save greenResults
	selectWindow("Results");
	saveAs("Results", dirOut + splitName[0] + "_greenResults.csv");
	...
	//counts synapses in each ROI and adds them to the Summary results table
    run("Analyze Particles...", "size="+colocMinPixel+"-Infinity pixel show=Overlay display exclude clear summarize");
	selectWindow("Summary");
	colocsCountArray = Table.getColumn("Count");
	
	//save colocResults
	selectWindow("Results");
	saveAs("Results", dirOut + splitName[0] + "_colocResults.csv");
	close("Results");
