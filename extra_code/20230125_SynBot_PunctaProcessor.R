...
for (i  in 1:length(redFiles)){
  ...
  if (grepl("KO", currentImage)){
    currentCondition <- "KO"
    if (KOcount == 0){
      redAreaVector_KO <- currentAreas
    }
    if (KOcount > 0){
      redAreaVector_KO <- append(redAreaVector_KO, currentAreas)
    }
    KOcount <- KOcount + 1
  }
  else{
...
redArea_KO.df <- data.frame(redAreaVector_KO)
colnames(redArea_KO.df) <- c("area")
redArea_WT.df <- data.frame(redAreaVector_WT)
colnames(redArea_WT.df) <- c("area")

ggplot(data = redArea_KO.df) + geom_histogram(aes(x = area), binwidth = 5) 
ggplot(data = redArea_WT.df) + geom_histogram(aes(x = area), binwidth = 5)

ggplot(data = redArea_KO.df) + geom_density(aes(x = area)) 
ggplot(data = redArea_WT.df) + geom_density(aes(x = area))

ggplot(data = redArea_KO.df) + geom_density(aes(x = area), fill = "magenta") + geom_density(data = redArea_WT.df, aes(x = area), fill = "red")
