...
manual_synbot_data[which(grepl("371", manual_synbot_data$Image)),]$condition <- "KO_sham"
...
manual_synbot_data[which(grepl("371", manual_synbot_data$Image)),]$animal <- "3-7-1"
...
manual_synbot_data$side <- "unknown"

manual_synbot_data[which(grepl("CONTRA", manual_synbot_data$Image)),]$side <- "CONTRA"
manual_synbot_data[which(grepl("IPSI", manual_synbot_data$Image)),]$side <- "IPSI"
manual_synbot_data[which(grepl("IPS", manual_synbot_data$Image)),]$side <- "IPSI"

manual_synbot_data$side <- factor(manual_synbot_data$side, levels = c("IPSI", "CONTRA"))
...
#calculate condition means for each animal
manual_raw_means <- summarize(group_by(manual_synbot_data, condition, animal), mean(Colocalized.Puncta.Count))

manual_raw_sum <- summarize(group_by(manual_synbot_data, condition, animal), sum(Colocalized.Puncta.Count))
...
#TODO: calculate animal averages
