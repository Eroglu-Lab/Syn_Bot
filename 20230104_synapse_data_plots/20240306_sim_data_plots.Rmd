...
#make analysis_type a factor so that it always plots in , manual, ilastik order
manual_synbot_data$analysis_type <- factor(manual_synbot_data$analysis_type, levels = c( "manual_synbot", "ilastik", "synquant"))
...
ggplot(data= combined_data, aes(x=condition, y=Colocalized.Puncta.Count)) + 
  ...
  geom_point(data = summarize(group_by(combined_data, analysis_type, condition, pair), mean(Colocalized.Puncta.Count)), aes(x = condition, y = `mean(Colocalized.Puncta.Count)`, color = condition), size = 3, position = position_dodge2(0.5)) +
