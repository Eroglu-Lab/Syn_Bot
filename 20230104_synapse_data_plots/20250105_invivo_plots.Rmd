...
#data can be downloaded from https://zenodo.org/records/12191805/files/a2d1_invivo_data.zip?download=1


manual_synbot_data <- read.csv("C:/Users/savag/Downloads/a2d1_invivo_data/20230119_juan_manual_cleaned.csv")
ilastik_data <- read.csv("C:/Users/savag/Downloads/a2d1_invivo_data/20230125_2_ilastik.csv")
puncta_analyzer_data <- read.csv("C:/Users/savag/Downloads/a2d1_invivo_data/20230406_chris_puncta_analyzer_cleaned_v2.csv")
synquant_data <- read.csv("C:/Users/savag/Downloads/a2d1_invivo_data/synquant_data_a2d1_combined_4.csv")
#SynQuant params:
# zscore_thres=10
# MinSize=10
# MaxSize=100
...
synquant_data[which(grepl("*WT*", synquant_data$Image)),]$condition <- "WT"
synquant_data[which(grepl("*KO*", synquant_data$Image)),]$condition <- "KO"

synquant_data$condition <- factor(synquant_data$condition, levels = c("WT", "KO"))
...
synquant_data[which(grepl(1, synquant_data$pair)),]$normalized_count <- synquant_data$Colocalized.Puncta.Count[which(grepl(1, synquant_data$pair))]/synquant_means$`mean(Colocalized.Puncta.Count)`[which(grepl(1, manual_raw_means$pair) & grepl("WT", synquant_means$condition))]

synquant_data[which(grepl(2, synquant_data$pair)),]$normalized_count <- synquant_data$Colocalized.Puncta.Count[which(grepl(2, synquant_data$pair))]/synquant_means$`mean(Colocalized.Puncta.Count)`[which(grepl(2, synquant_means$pair) & grepl("WT", synquant_means$condition))]

synquant_data[which(grepl(3, synquant_data$pair)),]$normalized_count <- synquant_data$Colocalized.Puncta.Count[which(grepl(3, synquant_data$pair))]/synquant_means$`mean(Colocalized.Puncta.Count)`[which(grepl(3, synquant_means$pair) & grepl("WT", synquant_means$condition))]
...
#normalize to mean of WT animals
puncta_analyzer_raw_wt_mean <- mean(filter(puncta_analyzer_means, condition == "WT")$`mean(Colocalized.Puncta.Count)`)
puncta_analyzer_means$global_norm <- puncta_analyzer_means$`mean(Colocalized.Puncta.Count)`/puncta_analyzer_raw_wt_mean

t.test(global_norm ~ condition, puncta_analyzer_means)


#paired t-test
WT_vector <- filter(puncta_analyzer_means, condition == "WT")$`mean(Colocalized.Puncta.Count)`
KO_vector <- filter(puncta_analyzer_means, condition == "KO")$`mean(Colocalized.Puncta.Count)`

t.test(WT_vector, KO_vector, paired = TRUE, alternative = "two.sided")

WT_vector <- filter(puncta_analyzer_means, condition == "WT")$global_norm
KO_vector <- filter(puncta_analyzer_means, condition == "KO")$global_norm

t.test(WT_vector, KO_vector, paired = TRUE, alternative = "two.sided")

#nested ANOVA

nest <- aov(`mean(Colocalized.Puncta.Count)` ~ condition / pair, puncta_analyzer_means)

summary(nest)

nest <- aov(`mean(Colocalized.Puncta.Count)` ~ condition + Error(pair), puncta_analyzer_means)

summary(nest)


nest_2 <- aov(global_norm ~ condition / pair, puncta_analyzer_means)

summary(nest_2)


nest <- aov(`mean(Colocalized.Puncta.Count)` ~ condition / pair, manual_raw_means)

summary(nest)

nest_2 <- aov(global_norm ~ condition / pair, manual_raw_means)
...
#simple t test
t.test(`mean(normalized_count)` ~ condition, puncta_analyzer_norm_means)

#paired t test
WT_vector <- filter(puncta_analyzer_norm_means, condition == "WT")$`mean(normalized_count)`
KO_vector <- filter(puncta_analyzer_norm_means, condition == "KO")$`mean(normalized_count)`
...
nest <- aov(`mean(normalized_count)` ~ condition / pair, puncta_analyzer_norm_means)
...
model_2 <- aov(`mean(Colocalized.Puncta.Count)`~condition, data = puncta_analyzer_means)
summary(model_2)

tukey_2 <- TukeyHSD(model_2)
tukey_2


model_3 <- aov(`mean(normalized_count)` ~ condition, data = puncta_analyzer_norm_means)
summary(model_3)

tukey_3 <- TukeyHSD(model_3)
tukey_3

model_4 <- aov(global_norm ~ condition, data = puncta_analyzer_means)
summary(model_4)

tukey_4 <- TukeyHSD(model_4)
tukey_4
...
#ggsave("F:/Savage_Dropbox/Duke Research Team Dropbox/Justin Savage/Team_Syn_Bot/20240318_figures/figure_s1_line_plots/invivo_coloc_line_plot_raw.pdf", width = 5, height = 2.75)
