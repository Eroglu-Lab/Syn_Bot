...
ggplot(data = output_df) + 
  ...
  geom_violin( aes(x = noise_level, y = recall))
...
ggplot() + geom_line(data = recall_means, aes(x = noise_level, y = `mean(recall)`))  + ylim(0.5, 1) + geom_errorbar()

precision_means <- summarize(group_by(output_df, noise_level), mean(precision))

ggplot() + geom_line(data = precision_means, aes(x = noise_level, y = `mean(precision)`))  + ylim(0.5, 1)
